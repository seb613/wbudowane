...
void task1(void *pdata)
{
    while (1)
    {
        ...
        // Post slider value to the queue
        OSQPost(SWQueue, (void *)sw);
...
void task2(void *pdata)
{
    ...
    while (1)
    {
        ...
        swstate = (int)OSQPend(SWQueue, 0, &err);
        if (err != OS_NO_ERR)
        {
    ...
    OSTimeDlyHMSM(0, 0, 1, 0); // sleep for 1 sec
}
...
int main(void)
{
    ...
    // Create the queue
    SWQueue = OSQCreate(SWQueueStorage, QUEUE_SIZE);
